...
<header class="site-head">
  <div class="mast-top"><div class="wrap mast-in">
    ...
    <a href="/"{' aria-current="page"' if current == "home" else ""} style="font-family:Georgia,serif;font-weight:700;font-size:clamp(20px,7vw,34px);letter-spacing:.14em;color:#a8752a;text-decoration:none">WRITERS</a></span>
...
def home() -> None:
    ...
    tools = [(t["id"], t["title"], t["short"], "") for t in _wtools]
    ...
    body = ('<style>:root{--brand:var(--brass)}</style>'
            + desk_hub_render.render(arts, tools, WR_CATS, _wr_cfg, "", TODAY_HUMAN and "Snapshot " + TODAY_HUMAN)
...
GUIDES = [
    ...
    {"slug": "how-much-to-charge-for-an-article", "title": "How much to charge for an article",
     "description": "Per-word, per-hour or flat fee: how to land on a number you can say out loud.",
     "deeper": ("/learn/freelance-paid-writing/how-much-to-charge-for-an-article/",
                "The full guide: how much to charge for an article"),
     "topics": ["rates", "pricing", "charge"], "toc": ["Three ways to price", "Know your floor", "The per-word trap", "The flat-fee advantage", "Say the number"],
     "body": "Pricing is the first business skill a writer needs, and the one nobody teaches.\n\n**Three ways to price.** Per word is the industry shorthand; per hour suits messy, open-ended work; a flat fee for the whole piece is usually the most profitable.\n\n**Know your floor.** Work out the least you can accept per hour without losing money - your target income, plus expenses, plus tax, divided by the hours you can actually bill.\n\n**The per-word trap.** Per-word rates quietly punish you for getting faster and better. A 500-words-per-hour writer at 10 cents a word earns 50 an hour; at the same speed, 25 cents earns 125.\n\n**The flat-fee advantage.** Quote the piece, not the word count: research, drafts and revisions included. Your speed becomes profit instead of a pay cut.\n\n**Say the number.** Practice one sentence: \u201cFor a piece like that, my fee is X.\u201d No apology, no range unless they ask."},
    {"slug": "the-tax-set-aside-habit", "title": "The tax set-aside habit for freelance writers",
     "description": "Freelance income arrives gross. This is the simple habit that keeps tax money out of your spending.",
     "deeper": ("/learn/freelance-paid-writing/the-tax-set-aside-habit/",
                "The full guide: the tax set-aside habit"),
     "topics": ["tax", "set aside", "money"], "toc": ["Why freelancers get hurt", "Pick a percentage", "Move it on payment day", "Expenses come first", "A planning estimate, not advice"],
     "body": "Freelancers rarely get hurt by owing tax - they get hurt by having spent it.\n\n**Freelance income arrives gross.** No employer withheld anything. Every payment that lands is partly someone else\u2019s money until you have set the tax share aside.\n\n**Pick a percentage.** A common starting band is 20 to 35 percent depending on where you live and what you earn. If you are not sure, 25 is a workable default - refine it with your tax authority\u2019s rules or an accountant.\n\n**Move it on payment day.** The same day a client pays, transfer the set-aside share somewhere you do not spend from. Willpower fails; mechanics do not.\n\n**Expenses come first.** Deductible business costs reduce the base you set aside from - track them all year, not in a panic later.\n\n**A planning estimate, not advice.** Rules differ by country. The point of the habit is simple: the money is already there when the bill arrives."},
    {"slug": "track-your-writing-income", "title": "Track your writing income like a business",
     "description": "What to log, why the pending column matters, and the client-concentration warning.",
     "deeper": ("/learn/freelance-paid-writing/track-your-writing-income/",
                "The full guide: track your writing income"),
     "topics": ["income", "tracking", "clients"], "toc": ["What gets measured gets negotiated", "Log four things", "Respect the pending column", "Watch client concentration", "Back it up"],
     "body": "Writers who track every payment spot the patterns that change businesses.\n\n**What gets measured gets negotiated.** A writer who knows last quarter\u2019s average per piece negotiates from data; one who does not negotiates from hope.\n\n**Log four things.** Client, amount, date, status - paid, invoiced or pending. Nothing else is needed to see the year clearly.\n\n**Respect the pending column.** Invoiced money is earned money you cannot touch. Watching that number is how you notice a client quietly going slow before it becomes a crisis.\n\n**Watch client concentration.** When one client passes half your paid income, you do not have a client - you have an employer with no obligations. Diversify deliberately.\n\n**Back it up.** Export your ledger somewhere safe now and then; a cleared browser cache should never cost you a year of records."},
    {"slug": "how-to-check-a-freelance-writing-contract", "title": "How to check a freelance writing contract",
     "description": "Rights, payment terms, revisions and kill fees - what to read before you sign.",
     "deeper": ("/learn/freelance-paid-writing/writing-contracts-what-to-check/",
                "The full guide: writing contracts - what to check"),
     "topics": ["contract", "agreement", "rights"], "toc": ["The five clauses that matter", "Rights and scope", "Payment and late terms", "Revisions and kill fees", "When there is no contract"],
     "body": "Most freelance disputes are not caused by bad clients - they are caused by unsigned expectations.\n\n**The five clauses that matter.** Scope (what exactly you deliver), rights (what they may do with it), payment (how much, when, and what happens when they are late), revisions (how many, and what counts as one), and kill fees (what you are paid if the project dies mid-flight).\n\n**Rights follow money.** Full buyouts cost multiples of one-time usage. If a client wants exclusive, perpetual, worldwide rights, the fee should say so. First rights only is the norm for articles.\n\n**Payment and late terms.** Net 30 is standard; anything past it earns a polite late fee. A contract that says nothing about late payment is a contract that quietly promises you will wait for free.\n\n**Revisions and kill fees.** One or two revision rounds, defined as comments on the delivered draft - not rewrites after new stakeholders appear. A kill fee of 30-50 percent on cancelled work is normal and fair.\n\n**When there is no contract.** The email thread where they agreed the fee, the length and the deadline IS your paper trail. Put it in one reply: \u201cConfirming: 1,200 words, X dollars, delivery Friday, one revision round.\u201d\n\nBRYME\u2019s agreement builder generates a clean starting contract in your browser - fill the fields, review the clauses above, and send."},
    {"slug": "how-writing-retainers-work", "title": "How retainers work for freelance writers",
     "description": "Monthly client agreements: what to include, how to price them, and the traps.",
     "deeper": ("/learn/freelance-paid-writing/how-writing-retainers-work/",
                "The full guide: how retainers work for writers"),
     "topics": ["retainer", "monthly", "pricing"], "toc": ["What a retainer really is", "Price the capacity, not the words", "The scope fence", "Terms that protect you", "The trap to avoid"],
     "body": "A retainer is the closest thing freelance writing has to a salary - and it only works when you define it like a contract, not a vibe.\n\n**What a retainer really is.** A monthly agreement: the client buys a defined block of your capacity - four articles, eight briefs, twenty hours - and you make yourself exclusively available for that block.\n\n**Price the capacity, not the words.** Add a premium to your normal rate (10-20 percent) because the client is buying certainty: your availability is reserved whether they use all of it or not. Unused months teach clients to treat retainers as discounts; unused days roll over only if you say so.\n\n**The scope fence.** List what is included and what is quoted separately - a retainer that says \u201cwriting support\u201d becomes unlimited support. Four blog posts a month, one revision round each, 48-hour turnaround: that is a fence.\n\n**Terms that protect you.** Payment on the first of the month, thirty days\u2019 notice to end, and a simple statement of what happens in a slow month. Retainers fail on vagueness, not on price.\n\n**The trap to avoid.** Taking on a retainer and letting it crowd out every other client. Half your capacity is the healthy ceiling - concentration beyond that is an employer with no obligations."},
     {"slug": "when-to-follow-up-on-a-pitch", "title": "When to follow up on a pitch (and when to stop)",
     "description": "Timing rules for the follow-up: how long to wait, what to send, how many times, and when to walk.",
     "topics": ["pitching", "follow-up", "editors"], "toc": ["The timing rule", "What to send", "How many times", "Signals to act on", "The habit that beats luck"],
     "body": "Silence after a pitch is not an answer, but it is information. The follow-up exists to catch the pitches that were buried, and it works only when it is short, calm and rare.\n\n**The timing rule.** One to two weeks, then one short follow-up. The publication's own submission page overrides everything - if it says four weeks or no reply, believe it.\n\n**What to send.** Three or four sentences on the original thread: still interested, still available, and a no is completely fine. That last sentence makes the reply easy.\n\n**How many times.** One follow-up, at a stretch two, a week apart. A third nudge is not persistence; it is a reputation.\n\n**Signals to act on.** A real no with feedback is a door - pitch that editor again. Three straight silences is a wall - spend the next idea elsewhere. If the publication announced a restructure or an editor left, the pitch is not buried, it is homeless: freshen the hook and send it to the next name on your list.\n\n**The habit that beats luck.** Pitch in batches. Five live pitches make one silence a statistic instead of a heartbreak."},
     {"slug": "how-to-price-ghostwriting-jobs", "title": "How to price ghostwriting jobs",
     "description": "Ghost rates sit above bylined rates: the invisibility premium, per-project quoting, and the three clauses that protect you.",
     "topics": ["ghostwriting", "pricing", "contracts"], "toc": ["Why the rate goes up", "Quote the project", "Price the input", "The clauses that protect you"],
     "body": "Ghostwriting is writing that carries someone else's name, and it has one pricing rule people keep getting wrong: ghost work should pay MORE than your bylined rate, not less.\n\n**Why the rate goes up.** A byline markets you while you sleep; ghostwriting pays once and builds nothing you can show. Price the invisibility - a defensible approach is 20 to 50 percent above your same-piece bylined rate. If a client says the credit is the payment, they are describing exposure, not a fee.\n\n**Quote the project, not the word.** Ghost jobs sprawl - vague briefs, extra voices, one more pass. A per-project quote with revisions capped at two rounds keeps the sprawl on the client's side of the fence.\n\n**Price the input.** Interviews, transcripts, voice-matching: hours a bylined piece never asks for. Name them in the quote (includes two 30-minute interviews) so the client sees what the number buys.\n\n**The clauses that protect you.** A portfolio clause (can you show redacted work?), a revision cap, and payment tied to the writing, not the publication. The rate calculator turns your target income into the baseline the ghost premium sits on."},
     {"slug": "where-the-money-is-in-writing", "title": "Where the money is in writing: an honest map of the pay",
     "description": "Articles, copy, technical docs, ghostwriting, staff jobs, screenwriting: how the work differs and where the pay actually sits.",
     "topics": ["income", "rates", "career"], "toc": ["The honest baseline", "Freelance articles and blogs", "Copy and content", "Technical and specialist", "Ghostwriting and screenwriting", "Staff jobs and the long game"],
     "body": "Every writing income conversation collapses into one question - what does it pay? - and the honest answer is a map, not a number. Here is the map.\n\n**The honest baseline.** The most cited public reference in the industry is the Editorial Freelancers Association\u2019s rate chart, rebuilt from member surveys every few years. The 2024 edition puts most professional writing work around 4 to 6 US cents per word, or roughly 50 to 70 US dollars an hour, with technical writing near the top of the band. Those figures describe experienced US-based freelancers; local markets differ, and newcomers start below them. What the chart is really for is calibration - it proves the market\u2019s floor is not zero.\n\n**Freelance articles and blogs.** The entry door. General-interest pieces cluster near the bottom of any range; niches move you up it - finance, health and technical writing reliably outpay \u201canything goes\u201d blogging, because the writer absorbs risk a generalist cannot. The arithmetic of pricing your own piece is on the how-much-to-charge guide, and the rate calculator turns your target income into a per-piece number you can say out loud.\n\n**Copy and content.** Writing that is measured against revenue - emails, landing pages, product copy - pays above article rates for the same reason salespeople eat first. It is also the natural home of retainers: one client, a reserved block of your month, a predictable invoice. The retainer guide explains the fence you need to build around it.\n\n**Technical and specialist.** Documentation, UX writing, white papers. Fewer gigs, stickier clients, top-of-band rates: the EFA\u2019s own chart places technical work at the high end precisely because the writer carries subject-matter weight a generalist cannot fake overnight.\n\n**Ghostwriting and screenwriting.** Ghost work should sit 20 to 50 percent above your bylined rate - the pricing logic is on the ghostwriting page. Screenwriting is the outlier on this map: unions publish minimums and the craft is real, but entry is gatekept by staffing and spec sales. Treat it as a second craft you build on the side, not a faster ladder.\n\n**Staff jobs and the long game.** A salary trades your ceiling for a floor; freelancing trades the floor for the ceiling. Neither is the correct answer - but whichever you pick, the mechanics are identical: track every payment (the income tracker keeps the ledger), set tax aside on payment day, and let the numbers, not hope, set your rates."},
     {"slug": "screenplay-format-basics", "title": "Screenplay format basics: the rules that make a script readable",
     "description": "Sluglines, action, dialogue, parentheticals: industry format explained in one pass - and why every rule exists.",
     "topics": ["screenwriting", "format", "script"], "toc": ["Format is readability", "The slugline", "Action lines", "Dialogue and parentheticals", "Transitions, pages and the one-minute rule", "Let the tools do the margins"],
     "body": "A screenplay is not prose. It is a working document read under time pressure by agents, readers, actors and crew - and format is how it earns a fair reading.\n\n**Format is readability.** Readers process thousands of pages. Industry format is a shared interface: when it is right, nobody notices; when it is wrong, the script reads as amateur before the story gets a chance. Every rule below exists for speed and clarity, not tradition alone.\n\n**The slugline.** Each new scene opens with a line in caps: INT. or EXT., the location, then DAY or NIGHT - INT. KITCHEN - NIGHT. It tells the unit where and when they are shooting before a single action line is read. A new location or time means a new slugline.\n\n**Action lines.** Present tense, only what the camera sees and the microphone hears. Paragraphs stay short - one to four lines. \u201cWe hear\u201d and \u201cwe see\u201d are usually cuttable; so are camera directions, unless you are directing your own script. White space is a feature: readers trust pages that breathe.\n\n**Dialogue and parentheticals.** The character\u2019s name in caps, the speech beneath it. Parentheticals - a short bracketed direction under the name - exist for a beat of HOW, not a mood board: (quietly), not (furiously stabbing the air). When the dialogue itself says it, delete the parenthetical.\n\n**Transitions, pages and the one-minute rule.** CUT TO: is mostly implied by the next slugline - spend transitions only where the cut itself is the point. The trade\u2019s rule of thumb is one page of properly formatted script to roughly one minute of screen time - approximate, not a stopwatch, but it is how everyone estimates. Twelve-point Courier remains the industry default typeface.\n\n**Let the tools do the margins.** Free and paid screenwriting software handles indentation, pagination and character spacing automatically; Fountain-style plain text does the same for writers who live in text editors. Use one. Format never makes a script good - but wrong format can stop it being read at all. The beat sheet shows you what to type once the margins are handled."},
     {"slug": "where-to-start-with-a-screenplay", "title": "Where to start with a screenplay: the beat sheet, explained honestly",
     "description": "The fifteen beats behind most produced films, credited to Blake Snyder - how to use the map without letting it use you.",
     "topics": ["screenwriting", "structure", "beats"], "toc": ["Logline first", "What a beat sheet is", "Act One: beats one to five", "Act Two: beats six to nine", "The road home: beats ten to fifteen", "How to actually use it"],
     "body": "Blank page, 110 pages to fill: that is why writers start with structure. The most used map in the industry is the fifteen-beat sheet from Blake Snyder\u2019s book Save the Cat! (2005).\n\n**Logline first.** One sentence: a hero, a goal, an obstacle. If the logline is dull, the script will be - write it before anything else and keep it above your desk.\n\n**What a beat sheet is.** A beat is a story event that turns the plot; the sheet lines up fifteen of them across three acts. It is a map, not a formula - produced films bend it constantly. Its real gift is momentum: fifteen small questions are answerable, 110 blank pages are not.\n\n**Act One: beats one to five.** Opening Image - the world before, in one frame. Theme Stated - someone says what the hero needs to learn, without knowing it. Set-Up - the life that is about to be broken, plus the stakes. Catalyst - the letter, the body, the announcement; the world moves. Debate - the hero resists the call, and the resistance is the character.\n\n**Act Two: beats six to nine.** Break into Two - the hero chooses the new world. B Story - the relationship that carries the theme (the partner, the mentor, the kid). Fun and Games - the promise of the premise, the set pieces the poster sold. Midpoint - false victory or false defeat, and the stakes turn personal.\n\n**The road home: beats ten to fifteen.** Bad Guys Close In - pressure from outside and from within the team. All Is Lost - the whiff of death, literal or symbolic. Dark Night of the Soul - the hero, emptied out, faces the theme stated back at them. Break into Three - the synthesis: what they could not do in Act One, they now can. Finale - the storm. Final Image - the world after, mirroring the opening frame.\n\n**How to actually use it.** One line per beat, on one page, before scene one. Treat each beat as a question - what is MY catalyst? - not a template to pour prose into. When a produced film you love breaks a beat, study why it worked; that is the difference between using the map and being used by it. Then open the format basics and write the pages the sheet earned you."},
]
