...
# ---- Batch #3 (2026-09-25): the counter-press doctrine ----
SPORT_ANALYSIS.extend([
    ('gegenpressing-explained', 'Gegenpressing: the six seconds that rebuilt football', 'Win the ball and counter-press instantly: where gegenpressing came from, why the six-second window matters, and what it costs the teams that live by it.', '<p>Every tactical idea gets a marketing name eventually, but gegenpressing earns its reputation in seconds, not slogans. The premise is almost insultingly simple: the most dangerous moment to win the ball is <b>the moment you win it</b>, because the opponent is disorganised, having just committed bodies forward to attack. So instead of securing possession and rebuilding, you hunt immediately &mdash; and the counter-attack you were supposed to defend never gets to exist.</p><h2>Where it came from</h2><p>The instinct is old &mdash; Dutch total football pressed in the 1970s, and Arrigo Sacchi\'s Milan made coordinated pressing a system &mdash; but <b>gegenpressing</b> (counter-pressing) as a doctrine is a German story: Ralf Rangnick\'s coaching tree, then J&uuml;rgen Klopp\'s Borussia Dortmund and Liverpool, who turned it into a title-winning identity. Klopp\'s framing became the canonical quote: <i>&ldquo;No playmaker in the world can be as good as a good counter-pressing situation.&rdquo;</i> The idea inverts the old coaching order: pressing was defence; now it is the first act of attack.</p><h2>The six-second rule</h2><p>The doctrine\'s most famous detail is a timer: if the ball is not won back within roughly <b>six seconds</b> of losing it, drop into your defensive shape and defend properly. The number is not magic; it is an honest reading of two facts. First, the opponent\'s vulnerability window is genuinely that short &mdash; the moment a passer finds a free man, the disorganisation ends. Second, pressing is brutally expensive: it runs on sprints, and sprints do not budget. The six-second rule is the sport\'s clearest example of a tactic being defined by its <b>stopping condition</b> rather than its trigger.</p><h2>What it demands &mdash; and what it costs</h2><p>Gegenpressing is a collective behaviour, not a striker\'s enthusiasm. It requires a compact shape (pressing with gaps is just jogging towards the ball), trigger discipline (everyone presses on the same cues &mdash; a bad touch, a closed-body reception, a pass into traffic), and a defensive line brave enough to sit high and squeeze the space behind the press. The costs are equally collective: enormous physical load and injury exposure, a squad deep enough to rotate it, and catastrophic exposure when the first press is beaten &mdash; because a team caught having pressed is a team caught out of shape. Teams that do it well concede almost nothing in transition; teams that do it half-committed concede exactly there.</p><h2>How it gets beaten</h2><p>The counters are now as coached as the press itself: bait the press with a goalkeeper or centre-back offering, then break the first line with a vertical pass into a pre-mapped free man &mdash; the possession logic <a href="/sports/possession-explained/">the possession explainer</a> describes. Third-man runs, one-touch lay-offs and switching play to the weak side all punish committed presses, and a team that beats the press once is playing against nine outfield players behind the ball. The modern equilibrium is why few teams press everywhere all the time anymore: the best sides press in <b>zones and moments</b> &mdash; high after losing the ball in the final third, mid-block otherwise &mdash; spending the six-second weapon where it pays, exactly as <a href="/sports/pressing-explained/">the pressing explainer</a> maps the wider family.</p>', [('Wikipedia &mdash; Gegenpressing', 'https://en.wikipedia.org/wiki/Gegenpressing'), ('Wikipedia &mdash; Total Football', 'https://en.wikipedia.org/wiki/Total_Football')], [('pressing-explained', 'Pressing, explained'), ('possession-explained', 'Possession, explained'), ('playing-out-from-the-back', 'Playing out from the back')]),
])
