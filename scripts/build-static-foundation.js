...
function managersPage(){
  ...
  const leagueBlocks = leagueOrder.filter(l => byLeague[l]).map(lg => {
    const rows = byLeague[lg].map(c => `<tr><td class="sp-mgr-club"><img src="${url('/assets/img/sports/club-' + c.id + '.svg')}" alt="Abstract crest for ${esc(c.name)}" width="28" height="34" loading="lazy"><b>${esc(c.name)}</b></td><td>${c.manager ? '<b>' + esc(c.manager) + '</b>' : '<span class="sp-mgr-pending">Pending verification</span>'}</td><td>${badge(c.note)} <span class="sp-mgr-note">${esc(c.note || '')}</span></td></tr>`).join('');
