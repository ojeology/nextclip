...
function verticalPage(v, category){
  ...
  const catGrid = (v.categories || []).map(c => `<a class="vcat" href="${url('/' + v.dir + '/' + c.slug + '/')}"><b>${esc(c.name)}</b><span>${esc(c.desc)}</span></a>`).join('') + (v.dir === 'memes' ? `<a class="vcat" href="${url('/memes/sticker-generator/')}"><b>✨ Sticker Generator</b><span>Turn your photo into a downloadable WhatsApp-ready sticker.</span></a>` : '');
...
const verticalPaths = ['/entertainment/','/memes/sticker-generator/'].concat(VERTICALS.flatMap(v => ['/' + v.dir + '/'].concat((v.categories || []).map(c => '/' + v.dir + '/' + c.slug + '/'))));
