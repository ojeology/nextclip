...
function matchCentre(){
  for (const lg of LEAGUE_FIX) {
    ...
    (F.matchweeks || []).forEach(w => w.matches.forEach(m => {
      ...
      if (lg.slug === 'premier-league' && m.id === 'arsenal' && m.away === 'coventry') {
        const review = [('Match overview', 'The season opener: defending champions Arsenal host Coventry City in Coventry’s first top-flight match in 25 years.'), ('Recent form', 'Research packet: Arsenal finished 2025/26 as champions on 85 points; Coventry won the Championship by 11 points under Frank Lampard.'), ('Head-to-head record', 'Historic meetings favour Arsenal. The supplied research records two Arsenal wins and no Coventry wins in the most recent recorded meetings.'), ('Last five meetings', 'Not applicable in a recent Premier League context: the sides have not met competitively in the current era.'), ('Home/away form', 'Arsenal were strong at the Emirates last season. Coventry’s away form contributed to their Championship title campaign.'), ('Key players', 'The review packet identifies Gabriel Jesus, Noni Madueke and Riccardo Calafiori for Arsenal, and Haji Wright as Coventry’s 17-goal promotion-season top scorer.'), ('Injuries', 'Provisional, recheck close to kick-off: Arsenal’s packet lists William Saliba, Ben White and Jurriën Timber as unavailable or in rehabilitation. Coventry: no reported injury concerns in the supplied snapshot.'), ('Suspensions', 'None reported in the supplied 14 August snapshot; must be reconfirmed before publication.'), ('Expected lineups', 'Provisional only: Arsenal — Raya; White, Mosquera, Gabriel, Calafiori; Guimarães, Lewis-Skelly; Ødegaard, Dowman, Havertz, Tzolis. Coventry are expected to use a compact defensive set-up.'), ('Tactical matchup', 'Arsenal’s World Cup call-ups mean a staggered pre-season, while Coventry had a normal off-season. The supplied analysis identifies fitness and Arsenal’s early cohesion as the key opening-day themes.'), ('Historical context', 'Coventry were Premier League founding members in 1992 and return after a long period outside the top tier following financial turmoil.'), ('Underdog discussion', 'Coventry are heavy underdogs, but the packet notes fresher legs, no European or World Cup disruption and the confidence of an 11-point title win.'), ('BRYME editorial outlook', 'Arsenal should control midfield and carry more threat in the box. The main risk identified is a patchwork return to fitness rather than a lack of tactical quality.'), ('Editorial score prediction', 'BRYME editorial prediction: Arsenal 2–0 Coventry City.'), ('Match result after the game', 'Pending — to be completed only after the officially confirmed result.'), ('Post-match analysis', 'Pending — to be completed after the fixture with verified match information.')];
        sections = review.map(x => { const post = /after the game|post-match/i.test(x[0]); return `<div class="sp-msec"><span class="sp-pend">${post ? 'After full-time' : 'Editorial review · 14 Aug 2026'}</span><b>${esc(x[0])}</b><p>${esc(x[1])}</p></div>`; }).join('');
      }
