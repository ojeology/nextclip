...
const VERTICALS = [
  {
    ...
    categories: [
      { slug: 'football', name: 'Football', desc: 'The home of football coverage — leagues, competitions and the beautiful game.' },
      { slug: 'premier-league', name: 'Premier League', desc: 'England\'s top flight: clubs, rivalries, records and history.' },
      { slug: 'champions-league', name: 'Champions League', desc: 'Europe\'s biggest club competition.' },
      { slug: 'la-liga', name: 'La Liga', desc: 'Spanish football: Real Madrid, Barcelona, Atlético and beyond.' },
      { slug: 'serie-a', name: 'Serie A', desc: 'Italian football and its storied clubs.' },
      { slug: 'bundesliga', name: 'Bundesliga', desc: 'German football, from Bayern to the Bundesliga\'s fan culture.' },
      { slug: 'international', name: 'International Football', desc: 'National teams, World Cups, continental tournaments and qualifiers.' },
      { slug: 'transfers', name: 'Transfers', desc: 'Transfer news, windows and how the market works.' },
      { slug: 'players', name: 'Players', desc: 'Player profiles, careers and stories.' },
      { slug: 'clubs', name: 'Clubs', desc: 'Club histories, identities and fan culture.' },
      { slug: 'history', name: 'History', desc: 'Historic moments, eras and football\'s greatest stories.' },
      { slug: 'records', name: 'Records', desc: 'Goals, titles, appearances and the numbers behind the game.' },
      { slug: 'articles', name: 'Sports Articles', desc: 'All long-form sports reading in one place.' }
    ]
  },
  {
...
function verticalPage(v, category){
  ...
  const catGrid = (v.categories || []).map(c => `<a class="vcat" href="${url('/' + v.dir + '/' + c.slug + '/')}"><b>${esc(c.name)}</b><span>${esc(c.desc)}</span></a>`).join('');
  const sportsFeature = (v.dir === 'sports' && !category) ? `<section class="sports-feature" aria-labelledby="sports-feature-title"><div class="sports-feature-inner"><div class="eyebrow">Featured story · BRYME Sports</div><h1 id="sports-feature-title">The season starts here.</h1><p>Fixtures, form, transfers and the stories that shape the Premier League — all in one focused home for the 2026/27 campaign.</p><a class="cta" href="${url('/sports/premier-league/')}">Explore Premier League coverage <span aria-hidden="true">→</span></a><div class="sports-feature-meta"><span>Premier League</span><span>Fixtures &amp; results</span><span>BRYME editorial</span></div></div></section>` : '';
  ...
  const pageHero = sportsFeature || `<section class="hero vhero vhero-${v.dir}"><div class="eyebrow">${v.emoji} ${category ? esc(v.name) + ' · ' + esc(category.name) : esc(v.name)}</div><h1>${esc(category ? category.name : v.name)}</h1><p class="lead">${esc(category ? category.desc : v.tagline)}</p></section>`;
  const body = `<main class="shell"><div class="crumb"><a href="${url('/')}">Home</a> / ${category ? `<a href="${url('/' + v.dir + '/')}">${esc(v.name)}</a> / ${esc(category.name)}` : esc(v.name)}</div>${pageHero}<section class="section">${category ? `<div class="vstate"><b>${esc(category.name)} — foundation ready</b><p>This section is being built. Articles will appear here as they are researched and published.</p></div>` : `<p class="lead" style="margin-bottom:18px">${esc(v.desc)}</p><div class="vnote">${esc(v.note)}</div><h2 style="margin:26px 0 14px">Explore ${esc(v.short)}</h2><div class="vcat-grid">${catGrid}</div>`}</section>${fixturesBlock}<section class="section"><div class="section-head"><h2>Explore BRYME</h2></div><div class="vchips">${VERTICALS.map(verticalChip).join('')}</div></section></main>`;
