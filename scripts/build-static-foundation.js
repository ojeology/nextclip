...
for (const [from, toList] of Object.entries(titleRelations)) {
  if (from === '_comment') continue;
  if (!slugIndex.has(from)) { warnings.push(`title-relationships.json: unknown key slug "${from}"`); continue; }
  (toList || []).forEach(t => { if (!slugIndex.has(t)) warnings.push(`title-relationships.json: unknown related slug "${t}" (from ${from})`); });
}
function relatedFor(m) {
  const typeDir = m.typeDir || 'movie';
  const manual = new Set(titleRelations[m.slug] || []);
  return movies
    .filter(x => x.id !== m.id && (x.typeDir || 'movie') === typeDir)
    .map(x => {
      let score = 0;
      if (manual.has(x.slug)) score += 100;
      if (typeDir === 'movie') { if (x.genre === m.genre) score += 4; }
      else { const shared = (m.genres || []).filter(g => (x.genres || []).includes(g)).length; score += shared * 2; }
...
for (const m of movies) {
  ...
  write(`${typeDir}/${m.slug}`, layout({
    ...
    schema: schemaList,
    image: poster(m),
    body: `<main class="shell"><div class="crumb"><a href="${url('/')}">Home</a> / <a href="${url('/' + (typeDir === 'movie' ? 'movies' : typeDir) + '/')}">${esc(typeDir === 'movie' ? 'Movies' : label)}</a> / ${esc(m.title)}</div>
  <section class="movie-hero" style="--movie-backdrop:url('${esc(poster(m))}')">
    ...
    <div>
      ...
      <div class="hero-actions">${m.trailer ? `<a class="cta" href="${esc(m.trailer)}" target="_blank" rel="noopener">Watch trailer on YouTube</a>` : ''}<a class="cta cta-ghost" href="#trailer">Play trailer</a><button class="quiet-link share-action" type="button" data-share-path="${pagePath}" data-share-title="${esc(m.title)}">Share</button></div>
    </div>
  </section>
  ...
  <section class="body">
    <article class="prose">
      ...
      <h2>Latest stories about ${esc(m.title)}</h2>
      ${relatedArticles.length ? `<div class="list">${relatedArticles.map(a => `<a class="row" href="${url('/article/' + a.slug + '/')}"><div><b>${esc(a.title)}</b><span class="meta" style="font-size:12px;color:var(--muted)">${esc(a.category)} · ${esc(a.description.slice(0, 110))}</span></div></a>`).join('')}</div>` : '<p>Related editorial reading is not available for this title yet.</p>'}
    </article>
    <aside class="aside">
      <h2>Details</h2>
      <dl><dt>Type</dt><dd>${esc(label)}</dd><dt>Year</dt><dd>${m.year || 'Information unavailable'}</dd><dt>Genre</dt><dd>${m.genres.length ? m.genres.map(g => `<a href="${url('/' + typeDir + '/' + slugify(g) + '/')}">${esc(g)}</a>`).join(', ') : (esc(m.genre || 'Information unavailable'))}</dd>${m.country ? `<dt>Country</dt><dd>${esc(m.country)}</dd>` : ''}${m.language ? `<dt>Language</dt><dd>${esc(m.language)}</dd>` : ''}${m.runtime ? `<dt>Runtime</dt><dd>${esc(m.runtime)}</dd>` : ''}${m.director ? `<dt>Director</dt><dd>${esc(m.director)}</dd>` : ''}${m.cast && m.cast.length ? `<dt>Cast</dt><dd>${m.cast.map(esc).join('; ')}</dd>` : ''}<dt>Year index</dt><dd><a href="${url('/year/' + m.year + '/')}">${esc(String(m.year))} movies</a></dd>${m.trending ? `<dt>Trending</dt><dd>🔥 #${m.trendingRank} (editorially curated)${m.trendingUntil ? ` · until ${m.trendingUntil}` : ''}</dd>` : ''}${m.popular ? `<dt>Popular</dt><dd>⭐ #${m.popularRank} (editorial popular pick)</dd>` : ''}${m.editorPick ? `<dt>Editor's Pick</dt><dd>👑 #${m.editorPickRank}${m.editorPickNote ? ` — ${esc(m.editorPickNote)}` : ''}</dd>` : ''}</dl>
    </aside>
  </section>
  ${relatedArticles.length ? `<section class="shell sub-section"><h2>READ OUR TAKE</h2>${relatedArticles.map(a => `<div class="take-card"><span class="eyebrow">${esc(a.category)}</span><h3>${esc(a.title)}</h3><p>${esc(a.description.slice(0, 130))}</p><a class="cta" href="${url('/article/' + a.slug + '/')}">READ OUR TAKE →</a></div>`).join('')}</section>` : ''}</main>`
  }));
  if (typeDir !== 'movie') {
