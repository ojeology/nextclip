  ...
  for (const lg of LEAGUE_FIX) {
    ...
    (F.matchweeks || []).forEach(w => w.matches.forEach(m => {
      ...
      write('sports/' + lg.slug + '/matches/' + slug(m), layout({
        noindex: !isEditorial,
        title: RES
          ? `${m.homeName} ${RES.homeScore}-${RES.awayScore} ${m.awayName} — Result & Analysis · ${F.league} ${F.season} | BRYME Sports`
          : (ED ? `${m.homeName} v ${m.awayName} — Preview, Form & Prediction · ${lg.roundLabel} ${w.number} | BRYME Sports`
                : `${m.homeName} v ${m.awayName} — ${lg.roundLabel} ${w.number} · ${F.league} ${F.season} | BRYME Sports`),
        description: RES
