...
function layout(o){
  ...
  <div class="footer-brand"><a class="brand" href="${url('/')}">NEXT<b>CLIP</b></a><p>Entertainment discovery for movies, TV series and anime.</p></div>
  <nav class="footer-col" aria-label="Explore"><h4>Explore</h4><a href="${url('/movies/')}">Movies</a><a href="${url('/series/')}">Series</a><a href="${url('/anime/')}">Anime</a><a href="${url('/articles/')}">Articles</a><a href="${url('/genres/')}">Genres</a></nav>
...
<p class="footer-note">NEXTCLIP · Movie, TV series and anime discovery with editorial guides. Trailer links lead to YouTube and viewing links lead to third parties.<small>Trending Now is editorially curated by NEXTCLIP — it is not live traffic data. Popular and Editor's Picks are independent rankings. Real user analytics will replace trending once the site has enough traffic. · Build ${new Date().toISOString().slice(0, 16).replace('T', ' ')} UTC</small></div></footer>${pageScript()}</body></html>`;
}
...
function heroSlideMarkup(m, idx){
  ...
  const rating = m.rating != null ? `<p class="hero-slide-rating">★ ${m.rating}/10 · NEXTCLIP Editorial</p>` : '';
...
write('years', layout({
  ...
  description: 'Browse NEXTCLIP years with strict content-type separation: movies by year, TV series by year and anime by year each have their own index.',
...
for (const [year, list] of yearMap) write(`year/${year}`, layout({
  title: `Movies from ${year}`, description: `Explore movies from ${year} in the NEXTCLIP catalogue.`, path: `/year/${year}/`, activeNav: 'movies', image: list[0] ? poster(list[0]) : undefined,
...
for (const [year, list] of seriesYearMap) write(`series/${year}`, layout({
  title: `Series from ${year}`, description: `Explore TV series from ${year} in the NEXTCLIP catalogue.`, path: `/series/${year}/`, activeNav: 'series', image: list[0] ? poster(list[0]) : undefined,
...
for (const [year, list] of animeYearMap) write(`anime/${year}`, layout({
  title: `Anime from ${year}`, description: `Explore anime from ${year} in the NEXTCLIP catalogue.`, path: `/anime/${year}/`, activeNav: 'anime', image: list[0] ? poster(list[0]) : undefined,
...
for (const m of movies) {
  ...
  const seoTitle = m.year ? `${m.title} (${m.year}) – ${typeWord}, Trailer & NEXTCLIP` : `${m.title} – ${typeWord}, Trailer & NEXTCLIP`;
  const genreText = m.genres.length ? m.genres[0] : (m.genre || '');
  const seoDesc = `Explore ${m.title}${m.year ? ' (' + m.year + ')' : ''} on NEXTCLIP — a ${m.typeLabel.toLowerCase()}${genreText ? ' ' + genreText.toLowerCase() : ''} overview with trailer, key details, related titles and our editorial take.`;
  ...
  <section class="movie-hero" style="--movie-backdrop:url('${esc(poster(m))}')">
    ...
    <div>
      ...
      <h1>${esc(m.title)}</h1>
      <div class="badges">${m.rating && m.rating.value != null ? `<span class="badge" title="NEXTCLIP editorial score — not IMDb, Rotten Tomatoes or audience ratings">★ ${esc(String(m.rating.value))}/10 · NEXTCLIP Editorial</span>` : ''}${m.trending ? `<span class="badge" title="Editorially curated — not live traffic data">🔥 Trending #${m.trendingRank}</span>` : ''}${m.popular ? `<span class="badge" title="Editorial popular pick">⭐ Popular</span>` : ''}${m.editorPick ? `<span class="badge" title="NEXTCLIP editor's recommendation">👑 Editor's Pick</span>` : ''}</div>
      <p class="lead">${esc(m.description || m.teaser || 'Information is being added for this title.')}</p>
      <div class="hero-actions"><a class="cta" href="#trailer">WATCH TRAILER</a>${relatedArticles.length ? `<a class="cta cta-ghost" href="${url('/article/' + relatedArticles[0].slug + '/')}">READ NEXTCLIP STORY</a>` : ''}<button class="quiet-link share-action" type="button" data-share-path="${pagePath}" data-share-title="${esc(m.title)}">Share</button></div>
    </div>
  </section>
...
const legalPages = [
  {
    ...
    title: 'About NEXTCLIP – Entertainment Discovery Platform',
    description: 'What NEXTCLIP is: an entertainment discovery platform for movies, TV series and anime — with trailers, editorial guides and curated recommendations.',
    hero: 'About NEXTCLIP', eyebrow: 'About', lead: 'An entertainment discovery platform for movies, TV series and anime.',
    sections: [
      { h: 'What NEXTCLIP is', p: ['NEXTCLIP is an entertainment discovery website focused on movies, TV series and anime. It helps visitors find titles worth watching by combining a curated catalogue, verified trailer links and original editorial articles.', 'The site is built around discovery: browse by content type, genre, year or country; search across the catalogue; read personal, opinionated articles; and watch official trailers where they are available.'] },
      { h: 'What you will find here', p: ['Movies, TV series and anime with clean title pages that include synopses, key details (year, country, language, genre), related titles and editorial links when they exist.', 'Trailers are linked or embedded from external platforms such as YouTube. NEXTCLIP does not host video files and does not claim ownership of third-party trailers, posters or clips unless explicitly stated.', 'Editorial articles are written by NEXTCLIP and reflect the personal opinions of their authors.'] },
      { h: 'Who is behind NEXTCLIP', p: ['NEXTCLIP is created and edited by its founder as an independent entertainment discovery project. Editorial articles are published under the author name shown on each article.'] },
      { h: 'A note on trailers and images', p: ['Trailer thumbnails and embedded players come from YouTube (using YouTube\'s privacy-enhanced domain for embeds). Availability of any trailer is controlled by the external platform and the uploader, not by NEXTCLIP. If a trailer becomes unavailable, the site shows a clean \u201cTrailer unavailable\u201d state instead of a broken player.'] }
    ]
  },
  {
    dir: 'contact', schemaType: 'ContactPage',
    title: 'Contact NEXTCLIP – Report Issues & Send Feedback',
    description: 'Contact NEXTCLIP about incorrect information, trailer or link issues, copyright concerns or general feedback.',
    hero: 'Contact NEXTCLIP', eyebrow: 'Contact', lead: 'Questions, corrections, feedback — we want to hear from you.',
    sections: [
  ...
  },
  {
    ...
    title: 'Privacy Policy – NEXTCLIP',
    description: 'NEXTCLIP privacy policy: what information is collected, how it is used, external services, cookies, analytics status and your rights.',
    hero: 'Privacy Policy', eyebrow: 'Legal', lead: 'Clear and honest information about privacy on NEXTCLIP.',
    sections: [
      { h: 'Information you provide voluntarily', p: ['If you contact NEXTCLIP by email or WhatsApp, you voluntarily provide the information contained in your message (such as your email address and the content of your enquiry). NEXTCLIP uses that information only to respond to you.'] },
      { h: 'Information collected automatically', p: ['NEXTCLIP is hosted on GitHub Pages. Like most web hosts, GitHub may process standard technical data (such as IP addresses and request logs) as part of operating the service. NEXTCLIP itself does not run analytics tracking on its pages.'] },
      { h: 'Cookies and local storage', p: ['NEXTCLIP does not set its own cookies for advertising or tracking, and does not rely on local storage for personal data. Standard browser mechanisms may operate as part of normal web use, but NEXTCLIP does not read or store personal information through them.'] },
      { h: 'Analytics', p: ['NEXTCLIP does not currently use Google Analytics or any other analytics service. If analytics is introduced in the future, this policy will be updated to describe it.'] },
      { h: 'External services and YouTube', p: ['Trailers are embedded from YouTube using the privacy-enhanced youtube-nocookie.com domain. When you play a trailer, YouTube\'s own privacy policy and cookie practices apply. NEXTCLIP has no control over YouTube\'s data handling.', 'Links to third-party websites (such as streaming platforms) leave NEXTCLIP; those websites have their own privacy policies.'] },
      { h: 'Advertising', p: ['NEXTCLIP does not currently display advertising and does not use advertising partners or advertising SDKs. If advertising or sponsored content is introduced later, it will be clearly identified and this policy will be updated.'] },
      { h: 'How information may be used', p: ['Information you send us is used to respond to your enquiry, improve the website and address reported issues. NEXTCLIP does not sell personal information.'] },
      { h: 'Data retention', p: ['Correspondence is kept only as long as needed to handle the enquiry. NEXTCLIP does not maintain user accounts or store visitor profiles.'] },
      { h: 'Your rights', p: ['You may contact NEXTCLIP at any time to ask what information we hold about you, to request correction or deletion, or to ask questions about this policy.'] },
  ...
  },
  {
    ...
    title: 'Terms of Use – NEXTCLIP',
    description: 'The terms governing your use of NEXTCLIP: acceptable use, content, external links, intellectual property and liability.',
    hero: 'Terms of Use', eyebrow: 'Legal', lead: 'The rules for using NEXTCLIP.',
    sections: [
      { h: 'Acceptable use', p: ['Use NEXTCLIP for lawful, personal, non-commercial discovery purposes. Do not attempt to disrupt the website, scrape it at scale, or use it in any way that could damage or overload the service.'] },
      ...
      { h: 'External links and embedded content', p: ['NEXTCLIP links to and embeds content from external platforms such as YouTube and third-party streaming services. NEXTCLIP does not control those platforms, their content, their availability or their policies.'] },
      { h: 'Accuracy limitations', p: ['While NEXTCLIP aims for accuracy, catalogue information, synopses, years, genres and trailer availability may contain errors or become outdated. Information should be verified independently before relying on it.'] },
      { h: 'Intellectual property', p: ['NEXTCLIP does not claim ownership of third-party trailers, posters, film footage or other copyrighted material belonging to studios, distributors or rights holders. All trademarks and copyrighted works belong to their respective owners.', 'Original NEXTCLIP editorial text and site content are the property of NEXTCLIP unless stated otherwise.'] },
      ...
      { h: 'Availability', p: ['NEXTCLIP is provided as a static website hosted on GitHub Pages. Availability depends on the hosting service and is provided on a best-effort basis without guarantee of uninterrupted access.'] },
      { h: 'Changes to the service', p: ['NEXTCLIP may change, add or remove features, content and pages at any time. These terms may also be updated; continued use of the site after changes means you accept the updated terms.'] },
      { h: 'Limitation of liability', p: ['NEXTCLIP is provided \u201cas is\u201d. To the maximum extent permitted by law, NEXTCLIP and its operator are not liable for any loss arising from the use of the site, reliance on its content, or the unavailability of third-party trailers or links.'] },
      { h: 'Contact', p: ['Questions about these terms can be sent to ' + LEGAL_CONTACT_EMAIL + '.'] }
    ]
  },
  {
    dir: 'disclaimer', schemaType: 'WebPage',
    title: 'Disclaimer – NEXTCLIP',
    description: 'NEXTCLIP disclaimer: an entertainment discovery platform; information may change, trailers are controlled by external platforms.',
    hero: 'Disclaimer', eyebrow: 'Legal', lead: 'Please read this before relying on anything you find here.',
    sections: [
      { h: 'Discovery platform', p: ['NEXTCLIP is an entertainment discovery and information platform. It helps visitors find movies, TV series and anime worth watching. It is not a streaming service and does not host video content.'] },
      { h: 'Information accuracy', p: ['Information on NEXTCLIP — including synopses, years, genres, countries and editorial claims — is provided in good faith but may contain errors or become outdated. Verify important information independently.'] },
      { h: 'Trailer availability', p: ['Trailers are provided by external platforms such as YouTube. Those platforms and their uploaders control the videos: a trailer can be removed, made private or restricted at any time. NEXTCLIP does not guarantee that any trailer will remain available, embeddable or playable.'] },
      { h: 'Third-party websites', p: ['Links to external websites lead away from NEXTCLIP. Those websites have their own terms, privacy policies and content. NEXTCLIP is not responsible for them.'] },
      { h: 'Editorial opinions', p: ['Articles on NEXTCLIP express the personal opinions of their authors. Rankings and recommendations are subjective unless explicitly stated otherwise.'] },
      { h: 'No professional advice', p: ['Nothing on NEXTCLIP constitutes professional, financial, legal or any other form of professional advice.'] }
    ]
  },
  {
    dir: 'copyright', schemaType: 'WebPage',
    title: 'Copyright & DMCA – NEXTCLIP',
    description: 'NEXTCLIP copyright notice and reporting procedure for rights holders concerned about potentially infringing material.',
    hero: 'Copyright / DMCA', eyebrow: 'Legal', lead: 'NEXTCLIP respects the rights of copyright owners.',
    sections: [
      { h: 'Our position', p: ['NEXTCLIP does not host video files, movies or television content. Trailers are linked or embedded from external platforms such as YouTube, and thumbnails are YouTube\'s generated preview images. NEXTCLIP does not claim ownership of third-party posters, trailers, screenshots or other copyrighted material — those belong to their respective owners.'] },
      { h: 'Reporting potentially infringing material', p: ['If you are a rights holder and believe material on NEXTCLIP infringes your copyright, contact us with the following information:', '1. The exact URL(s) on NEXTCLIP where the material appears.', '2. Identification of the copyrighted work you believe is affected.', '3. Your relationship to the material (for example, rights holder or authorized representative).', '4. Contact details so we can reply to you.'] },
      ...
      { h: 'What happens next', p: ['NEXTCLIP reviews each report on its merits. Where a trailer or thumbnail is hosted by a third party such as YouTube, we may also point you to that platform\'s own reporting tools, since they control the underlying content.'] }
    ]
  },
  {
    dir: 'editorial-policy', schemaType: 'WebPage',
    title: 'Editorial Policy – NEXTCLIP',
    description: 'How NEXTCLIP articles are written: entertainment-focused, opinionated where labelled, fact-checked, correctable and transparent about sponsored content.',
    hero: 'Editorial Policy', eyebrow: 'Information', lead: 'How NEXTCLIP writes and publishes its articles.',
    sections: [
      { h: 'Purpose', p: ['NEXTCLIP articles are written for entertainment and discovery purposes: to help readers find something worth watching, understand why a title works, and compare options. They are not academic reviews and not objective buyer-style rankings unless explicitly stated.'] },
      ...
      { h: 'Facts and sources', p: ['Where articles state factual information — release years, episode counts, plot basics — NEXTCLIP aims for accuracy and checks facts against reliable sources. External sources may be referenced where appropriate. If a specific fact cannot be verified, it is not invented.'] },
      { h: 'Distinguishing opinion from fact', p: ['NEXTCLIP tries to keep the distinction clear: descriptive information (what a series is about, when it aired) is presented as information, while judgments (whether it is good, which one is better) are presented as the author\'s opinion.'] },
      { h: 'Corrections', p: ['When errors are discovered — in facts, links or metadata — NEXTCLIP will correct them as soon as practical. Readers are encouraged to report errors via the Contact page.'] },
      { h: 'Sponsored content and advertising', p: ['NEXTCLIP does not currently publish sponsored content or advertising. If sponsored or paid content is introduced in the future, it will be clearly identified as such so readers always know what they are reading.'] }
    ]
  }
];
