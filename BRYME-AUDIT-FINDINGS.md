...
|---|----------|-------|------|----------|--------------|--------|
| A5 | P2 | Zero JSON-LD on Money desk pages and Sports explainer pages, while Entertainment guides carry Article+Organization and Writers carries the full set | `/money/` hub, `/money/<guide>/` (90), `/sports/<slug>-explained/` | JSON-LD sample scan of built trees, 2026-09-26 | Add Article (guides) / CollectionPage (hubs) + Organization publisher via the generators — renderer-level, no per-page hand-edits | **OPEN** |
