...
## Batch 66 — 15 Sep 2026 (THE FORMER ENTERTAINMENT RESTORED - owner: "check my repository... bring it back; entertainment shouldn't be a card, it should be beautiful")
- FORENSICS: git history held the full NEXTCLIP movie platform (651 commits). Restored from 6e9fa5ba2a~1: data/movies.json 726 published titles (695 with youtubeIds), rich fields (cast/director/runtime/BRYME editorial score/facts/teaser + honest labelled search watchLinks on 32), full old CSS (home-hero, feature rails, genre grid, movie-hero, trailer frame).
- VERIFICATION: all 690 unique ids re-verified via oEmbed (690/690 live, title+channel). 9 studio-channel overrides (b65 hand-checks) applied where the old id was a media channel (e.g. Parasite IGN -> NEON). Pages print channel + "link verified 15 September 2026" + "nothing loads until you press play".
- BUILT: /entertainment/browse/ = the platform (hero, trending rail, genre grid + 25 genre shelves incl. Nigerian/Indian/Chinese, recent favourites, desk reading, verified-trailers band; 224KB) + /entertainment/movie/<slug>/ x 726 (movie-hero with backdrop, badges, 16:9 click-to-play facade, story, facts, watch-searches, aside, related desk guides by genre). assets/trailer-facade.js v2 (modestbranding, referrerpolicy). jsdom click test green.
- ALLOWLIST LESSON: source allowlist = WRITERS-root-relative only; my +726 "/movie/" entries became phantom /writers/movie/ routes (2449) - reverted to 505; entertainment routes auto-flow (v26 = 1723 exactly).
- BUILDER FIX: build-writing-first legacy_redirect_stubs wrote a noindex "Page moved" stub at /tech/ (b17 relic) - order-dependently fought the ecosystem mirror and crashed build-discovery ("allowlisted route not index,follow"). Stub line REMOVED permanently; /tech/ hub verified real again.
- Chain: 1,735pp / 141,947 links / ok:true / v26 1723. Commit 6b3df0f0 push MATCH. Live: movie pages + tech hub serve b66; browse served stale from CDN edge at first check (origin proven correct via committed bytes) - expected to expire; re-verify.

## Batch 66b/c — 15 Sep 2026 (RENDER DEPLOY ROOT-CAUSE FIXED; platform fully live)
- SYMPTOM: every b66 deploy build_failed while movie pages served but browse served the cached b65 file (mixed CDN state).
- ROOT CAUSE (reproduced in a clean clone): Render buildCommand = `npm install --omit=dev && npm run build`. The npm build regenerated PRE-routing trees, then the unconditional `&& python3 scripts/check-internal-links.py` tail saw 10,159 "broken" links (links that build-routing rewrites later) and exited 1 -> build_failed. b65 passed only because its trees happened to stay consistent; the /tech/ stub removal exposed the real ordering fragility.
- FIX (f9397fa9): npm build now chains build-routing BEFORE build-public-dir and drops the unconditional check-links tail. Render publishes the committed, locally-validated public/. Validation law unchanged: the full chain (incl. check-links) runs in the workspace before every push.
- RESULT: dep-dajrg2aj (f9397fa9) LIVE. Browse = platform (213KB served, hero/rails/shelves/tiles/thumbs all present), movie pages 200 with trailers + runtime fix ("min min" gone), home + tech hub healthy. Pings re-fired.
- LESSON LAW: any builder output that depends on build-routing MUST chain routing inside the same pipeline; a validator tacked after an unguarded `||` tail fails every deploy when trees are pre-routing. Local pre-push validation remains the real gate.

## Batch 66c — 15 Sep 2026 (LEGACY-LINK FORWARDS + STAGING FIX; owner saw stale cards + 404s)
- OWNER REPORT: entertainment "looks the same, all cards" + clicking pages shows "not available". DIAGNOSIS: the owner's browser held the pre-restore cached catalogue (old card grid) whose root-style links (/slug/) 404 on the routed property layout. Live origin was already correct (verified).
- FIX 1 (63 forwards): build-routing now generates noindex meta-refresh stubs at /<slug>/ for every entertainment content slug -> /entertainment/<slug>/ (63 stubs; skip list guards legal/about dirs + movie; never overwrites existing root pages). Old cached links now auto-forward everywhere.
- FIX 2 (staging correctness): build-public-dir's five-publication override copied UNREWRITTEN ecosystem trees over routing's rewritten mirror (local-only clobber - Render clones have no ecosystem/config.json so the override never fired there; live was never affected). Override now prefers the routed ROOT tree. Local chain order law: build-ecosystem BEFORE npm build (npm build now contains routing).
- Chain: 1,798pp (incl. 63 stubs) / 142,073 links / ok:true / v26 1723. Commit c2d2080d push MATCH -> deploy LIVE. Verified: browse 224KB hero present, forward stub serves, movie pages 200, ping 200.
- Also this turn: .git drops #32/#33 (rebuilt, zero losses); one more /home/user cwd slip (fail-fast, zero writes - 3rd; in-command `cd || exit 1` guard now standard on every repo call); a transient writing-first mid-run crash (single occurrence, unreproducible, clean-chain rerun green).

## Batch 67 — 15 Sep 2026 (owner: slim header + monetag balance + shelves as landing + FITNESS HUB)
- HEADER: entertainment mast slimmed (b67 slim-mast style on every entertainment page: tighter padding, clamp(19-24px) brand, edition hidden <=700px, the long wordmark hidden <=480px -> brand reads BRYME on phones).
- LANDING SWAP: /entertainment/ IS the platform (shelves/hero/rails). Old desk home retired; /entertainment/browse/ = noindex meta-refresh stub -> /entertainment/; 23 stale hrefs swept to root; nav "Browse movies" -> root; movie crumbs/backlinks -> root.
- MONETAG v3: vignette cap relaxed 1 -> 2 per 12h (owner asked "not too strict and not annoying"; zone2 stays 4/24h). Carriers ?v=3 both builders.
- FITNESS HUB: +4 pages via FIT_MORE: benefits-of-morning-water, benefits-of-sleeping-well, caffeine-side-effects, 30-day-weight-loss-programme (owner-required "results are not guaranteed" + not-medical-advice disclaimers throughout; all numbers labelled approximate). related_map wiring + 4 curated hub rows (Fuel/Recover/Program labels).
