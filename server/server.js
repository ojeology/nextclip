...
/* The deployed artifact. render.yaml publishes `public` (staticPublishPath:
   public) as a Render static site, so that directory -- not the repo root -- is
   what production serves. Override with PUBLISH_DIR to point elsewhere. */
const ROOT=path.resolve(__dirname,"..",process.env.PUBLISH_DIR||"public");
...
/* A static host serves whatever sits in the publish directory, so this is
   ...
   pre-routing writers directories (writing, guides, tools, learn, ...) and so
   404'd every routed path -- /writers/, /tech/, /sports/, /entertainment/,
   /fitness/, /home/ -- which is why the HTTP and browser gates could not see
   the site that actually ships. Dot-directories stay denied by safeFile. */
const PUBLIC_HTML_DIRS=new Set((()=>{try{return fs.readdirSync(ROOT,{withFileTypes:true}).filter(d=>d.isDirectory()&&!d.name.startsWith(".")).map(d=>d.name)}catch{return[]}})());
...
/* Documents that a published directory may hold. Without these, /writers/sitemap.xml
   and /tech/sitemap.xml 404 -- and robots.txt declares /writers/sitemap.xml to
   crawlers while sitemap_index.xml submits all seven to Search Console. */
const PUBLIC_DOC_EXT=new Set(["",".html",".xml",".txt",".json",".webmanifest"]);
...
function safeFile(rel){
 ...
 else if(PUBLIC_ASSET_EXT.has(path.extname(parts.at(-1)).toLowerCase()))allowed=true;
 else if(PUBLIC_HTML_DIRS.has(parts[0])&&(parts.length===1||PUBLIC_DOC_EXT.has(path.extname(parts.at(-1)).toLowerCase())))allowed=true;
...
const server=http.createServer((req,res)=>{
 ...
 if(/\/index\.html$/i.test(raw)){const target=raw.replace(/index\.html$/i,"")||"/";
  /* Canonicalise only when the directory actually resolves. Answering 308 first
     and 404ing afterwards would hide a containment miss behind a redirect. */
  if(safeFile(target==="/"?"index.html":target.replace(/^\//,""))){res.writeHead(308,headers({location:target+(url.search||""),"cache-control":"public, max-age=86400"}));return res.end()}}
 ...
 /* A published file wins over a redirect rule, which is what the static host
    does: /tech/paypal-vs-wise-vs-payoneer/ returns 200 in production even
    though a /tech/* -> /guides/ rule exists. Rules fire only for paths that
    have no file, so they can no longer shadow real pages. */
 ...
 /* 410 needs a Gone page to serve. The published artifact has no 410.html, so
    retired media families fall through to the same 404 production returns. */
 if(mediaGone(raw)){const gone=pageFile("410.html");if(gone)return sendFile(res,gone,410,method)}
